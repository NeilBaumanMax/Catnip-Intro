...
**文档漂移修正完成后，必须将当前分支上传至 GitHub 作为备份。** 具体步骤：

1. 确认文档漂移已全部修正且本地无遗漏。
2. `git add` 所有修正后的文档文件。
3. `git commit` 提交文档漂移修正。
4. `git push origin <当前分支名>` 推送至 GitHub。
5. 在 LOG.md 的文档漂移检查区域记录备份分支名和 commit id。

如果当前分支尚未推送到 GitHub（新分支），推送时会自动创建远程分支。如果本轮不允许提交或推送，必须在 LOG.md 中记录不能上传备份分支的原因。

...
- 毛玻璃导航 + 全页背景图 + 玻璃卡片
- 4 产品 Hero 轮播（后端 SSE 驱动，多设备同步）
- 联系我们词云动效（JS 动态位置+颜色+大小）
- Orbitron 科技风字体
- 移动端适配（响应式间距、图片可见）
- Logo 透明 PNG（去白底）
...
- 统一 JSON 响应（internal/api/response.go）。
- CORS 中间件（Allow-Origin: localhost:3000）。
- 内存 token store（crypto/rand + sync.RWMutex）。
- bcrypt 密码哈希（golang.org/x/crypto/bcrypt）。
- POST /api/auth/login（200 / 401）。
- Bearer token 鉴权中间件（RequireAuth）。
- GET /api/admin/ping（受保护）。
- db-seed 默认管理员：admin / admin123456（bcrypt）。
...
- 通过：health、login success、login fail 401、no token 401、with token 200、bad token 401、uploads regression。
- 失败项：无。

文档漂移：已修正。
限制事项：写入业务代码（登录鉴权底座），安装 golang.org/x/crypto v0.53.0。
下一次施工建议：Phase 4B（产品/案例/留言/网站设置 CRUD）。

接手前必须读取：docs/LOG.md、HANDOFF.md、DEV_PROGRESS.md、backend/cmd/server/main.go、backend/internal/auth/、backend/internal/middleware/。

...
后端技术栈：Go 1.25.0 + database/sql + modernc.org/sqlite（纯 Go，无 CGO）。
...
- 开发阶段计划已更新（Phase 0-9）。
- 标准施工流程已定义。
- 后端技术栈决策文档（`BACKEND_TECH_STACK_DECISION.md`）。
- Go backend 最小骨架：`GET /health` 可用。
- Go + SQLite 数据库接入：
  - `internal/database/database.go`（连接管理）
  - `internal/database/schema.go`（5 张表 DDL，snake_case 列名）
  - `internal/models/`（Admin、Product、Case、Message、SiteSetting）
  - `cmd/db-seed/main.go`（ON CONFLICT upsert，幂等可重复执行）
  - `cmd/db-check/main.go`（count + 样例）
...
- 尚未创建前端项目（Next.js）。
- 尚未实现任何页面。
- 尚未实现 HTTP 业务接口（登录、产品、案例、留言、上传）。
- 尚未实现图片上传。

## 下一位 Codex 应做什么

进入 Phase 3 或 Phase 4 前，先执行备份，读取工程文档。

建议下一阶段实现 Go backend HTTP 业务接口，或先完成 uploads 图片上传。

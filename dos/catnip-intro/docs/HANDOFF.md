...
- 测试和验收标准已定义。
- 标准施工流程已定义。
- 后端技术栈决策文档已创建（`BACKEND_TECH_STACK_DECISION.md`）。
- 数据库迁移策略文档已更新（`DATABASE_MIGRATION_STRATEGY.md`）。
- Node/Prisma 历史代码保留为已验证参考资料。
...
- `backend/` 中 Node.js + Express + TypeScript + Prisma 代码，Phase 1-2B 已验证。
- `data/company.db` 已有 5 张表结构，作为 Go 后端建表参考。
- `prisma migrate` 不可用问题已记录为历史问题，不再阻塞项目。

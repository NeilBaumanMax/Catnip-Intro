...
后端技术栈：Go 1.24.5 + net/http（标准库），零第三方依赖。
...
- Go backend 最小骨架已创建：
  - `backend/go.mod`（Go 1.24.5）
  - `backend/cmd/server/main.go`（net/http）
  - 监听 `0.0.0.0:4000`
...
进入 Phase 2E 前，先执行备份，读取工程文档。

Phase 2E 应在 Go backend 中接入 SQLite，创建数据库表结构并验证读写路径。
